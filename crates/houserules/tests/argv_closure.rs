//! Pins the bare `--dir` divergence class across the whole flat CLI
//! surface: clap's own value-required error fires on every command, a
//! shape the frozen JS never has at all (`tools/kb.mjs`/`tools/backlog.mjs`
//! accept no `--dir`; it is this binary's own testability addition).
//!
//! Every other argv shape (unknown flags, duplicated value flags,
//! unexpected positionals) has its own pin, per command, in that command's
//! own `_parity.rs` file: `backlog_parity.rs` for
//! `list`/`get`/`batch`/`set`/`check-backlog`, `check_parity.rs` for
//! `check-knowledge`, `validate_stats_audit_parity.rs` for
//! `audit`/`validate`/`stats`, `read_parity.rs` for
//! `index`/`for`/`topics`/`standing`, and `render_parity.rs` for `render`.
//! No command in the flat surface is exempt from this class: probing
//! `check-knowledge` and `check-backlog` the same way as every other
//! command (`tools/kb.mjs check --bogus`/`extra`, `tools/backlog.mjs check
//! --bogus`/`extra`) finds the same JS-ignores-it-and-succeeds divergence
//! the rest have, even though both take only `--dir` beyond their own name.
//! This file is only the one shape common to all fourteen.
//!
//! This test itself needs no fixture: clap's own argument-parsing pass
//! fails on a bare `--dir` before `main` ever dispatches to a command, so
//! the binary's answer (exit 2, the same message, naming that one flag) is
//! unconditional -- true with every other argument omitted, which is what
//! this test actually gives it.
//!
//! `--dir X --dir Y` (both copies carrying a value) is common to all
//! fourteen commands the same way a bare one is, so it belongs beside it,
//! not in any one command's own `_parity.rs` file. The binary's half needs
//! no fixture either, for the same structural reason as the bare case:
//! `dir: Option<PathBuf>` is a plain, non-multiple clap argument on every
...
//! command's OTHER arguments are even present. The JS half rests on
...
//! irrelevant to the flag-handling divergence this test and the one above
//! it both pin.
...
/// surface. Every one of these instead runs normally under a plain,
/// unqualified `--dir` in JS (there being no such flag to begin with,
/// `--dir` is absorbed as a harmless, ignored unknown option), so this
/// binary's own uniform failure here is the whole of the divergence class,
/// not a coincidence of any one command's own logic.
#[test]
...
/// surface. JS's `parseArgs` (`tools/lib/cli.mjs`) consumes each `--dir`
/// occurrence as a value for the same `opts.dir` key, the second
/// overwriting the first, and never reads it, so both are absorbed
/// without error there too.
#[test]
