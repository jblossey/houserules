...
//! 1): the golden byte-compare (mini and root bases) and the `render
//! --check` stdout/exit contract, exercised against the compiled binary
//! the same way `houserules render` runs.
//!
//! The byte-compare targets moved from the frozen fixture corpus
//! (`tests/corpus/render/`) to reviewed, Rust-generated goldens
//! (`tests/goldens/render/`, regenerated by `cargo run --bin gen-goldens`)
//! at batch 18 T5 (docs/specs/2026-09-05-batch-18-phase3.md §3): `render`'s own
//! generated output now embeds the flat `houserules` command surface, which the
//! frozen JS at the (now retired) `tests/corpus/manifest.json`'s `frozen_sha`
//! could no longer produce. `diff-shape-gate` proved the re-baseline was only
//! that rewrite, before it retired as a spent one-time proof (HR-093). The
//! comparison logic below is otherwise unchanged: same fixture, same frozen sha
//! (`common::FROZEN_SHA`, since batch 20 T3), same file-set-then-bytes
//! assertions.
...
/// Tests (brief item 2): renders against `tests/fixtures/mini/`
/// and byte-compares every produced file against `tests/goldens/render/mini/**`
...
/// Tests (brief item 3): renders against a detached worktree at the
/// frozen sha (`common::FROZEN_SHA`), and byte-compares every produced
/// file against `tests/goldens/render/root/**`. Also proves the ordering
/// trap (parity trap 1): `areas.json` declares `docs` before `cli` before
