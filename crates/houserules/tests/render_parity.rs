...
//! which the frozen JS at `tests/corpus/manifest.json`'s `frozen_sha`
//! cannot produce. `diff-shape-gate` (`cargo run --bin diff-shape-gate`)
//! proves the re-baseline is only that rewrite. The comparison logic
//! below is otherwise unchanged: same fixture, same frozen sha, same
...
/// `tests/goldens/render/root/**`. Also proves the ordering trap (parity
