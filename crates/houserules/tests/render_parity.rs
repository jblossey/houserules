...
//! at batch 18 T5 (docs/specs/2026-09-05-batch-18-phase3.md §3): `render`'s
//! own generated output now embeds the flat `houserules` command surface,
//! which the frozen JS at the (now retired) `tests/corpus/manifest.json`'s
//! `frozen_sha` could no longer produce. `diff-shape-gate` (`cargo run
//! --bin diff-shape-gate`) proves the re-baseline is only that rewrite.
//! The comparison logic below is otherwise unchanged: same fixture, same
//! frozen sha (`common::FROZEN_SHA`, since batch 20 T3), same
//! file-set-then-bytes assertions.
