...
//! `validate` (all four slices) and `stats` (both slices) against
//! `tools/kb.mjs`'s captured output; FIELD-identical JSON (spec §4) for
//! `audit` against both audit slices, compared as parsed `serde_json::Value`
//! rather than raw bytes -- a JSON object's key order is not part of its
//! value (this crate's `preserve_order` feature backs `Value::Object` with
//! an `IndexMap`, whose `PartialEq` is itself order-independent), so this
//! is the correct comparison for "same fields, same values, same row
//! order" without demanding a byte-identical `area_files` key order the
//! frozen JS's own insertion-order artifact never promised either
//! (`rules::audit`'s own module doc has the fuller account of that
//! specific field).
//!
//! Every slice moved from the frozen fixture corpus (`tests/corpus/
//! {validate,stats,audit}/`) to reviewed, Rust-generated goldens
//! (`tests/goldens/{validate,stats,audit}/`, regenerated by `cargo run
//! --bin gen-goldens`) at batch 20 T3 (HR-047, docs/specs/2026-09-07-
//! batch-20-phase5.md §2's boundary correction; design.md §5.42):
//! `tests/corpus/**` retires with the rest of the vitest layer this same
//! batch. The comparison logic below is otherwise unchanged.
//!
