//! Backlog command parity tests: five backlog slices (`list --open`,
//! `get HR-052`, `batch 14`, `check`, `set`), byte-compared verbatim
//! against the compiled binary, plus the CLI's usage-error and
//! load-failure arms (`tools/backlog.mjs`'s `main`, ported).
//!
//! The five slices sit in reviewed, Rust-generated goldens
//! (`tests/goldens/backlog/`, regenerated by `cargo run --bin
//! gen-goldens`). The comparison logic below is otherwise unchanged --
//! same fixture, same frozen sha, same byte compares.
//!
//! The `check` slice pins a finding, not a clean run: the frozen sha's
//! own `backlog/parked.json` predates the emitter round-trip gate and
//! does not round-trip, so `tests/goldens/backlog/check.json` pins that
//! one finding and exit 1, not `backlog: ok`/exit 0 -- see
//! `check_backlog_matches_the_frozen_corpus`'s own doc. `tests/fixtures/
//! mini` round-trips cleanly, so the ok path stays pinned by
//! `check_backlog_matches_the_mini_corpus`, one clean slice next to one
//! failure slice.
...
/// One reviewed `tests/goldens/backlog/<slice>.json` capture: `stdout`,
/// `stderr`, and `exit`, read directly from the golden file so a golden
...
fn corpus_run(relative: &str) -> CorpusRun {
    let text = fs::read_to_string(repo_root().join(format!("tests/goldens/backlog/{relative}")))
        .unwrap_or_else(|error| panic!("read tests/goldens/backlog/{relative}: {error}"));
    let value: serde_json::Value = serde_json::from_str(&text).expect("parse golden backlog slice");
    CorpusRun {
...
/// Runs against the frozen worktree: `list --open`.
#[test]
...
/// Runs against the frozen worktree: `get HR-052`.
#[test]
...
/// Runs against the frozen worktree: `batch 14`.
#[test]
...
/// `check-backlog` against the frozen sha's own backlog: not a clean run
/// -- `tests/goldens/backlog/check.json` pins `backlog/parked.json: does
/// not round-trip through the shipped emitter ...`, empty stdout, exit
/// 1, because the frozen sha's own `backlog/parked.json` predates the
/// round-trip gate and was hand-formatted with single-line item objects.
/// The same finding prints verbatim every time, since the frozen sha
/// never moves. The clean path stays pinned by
/// `check_backlog_matches_the_mini_corpus` below.
#[test]
...
/// The clean path `check-backlog`'s own output stays golden-pinned even
/// though the frozen-sha slice above now pins a failure --
/// `tests/fixtures/mini` round-trips byte-for-byte, so `backlog: ok`,
/// exit 0 still has a slice proving it.
#[test]
...
/// Runs `set` on a fresh copy of the `mini` fixture (never the live tree),
/// then byte-compares both the captured stdout and the WRITTEN FILE against
/// the frozen corpus -- the write path is the first Rust code that mutates
/// a repository file. The fixture's two items exercise both write shapes at
/// once: `HR-901`'s `status` updates an existing key in place and its
/// `batch` is a brand new trailing key; `HR-902` is untouched and must
...
fn set_matches_the_frozen_corpus_stdout_and_written_file() {
    ...
    let expected_bytes =
        fs::read(repo_root().join("tests/goldens/backlog/set/mini/backlog/items/misc.json"))
            .expect("read golden written-file capture");
    assert_eq!(
        written, expected_bytes,
        "set: written backlog/items/misc.json diverged from the golden byte-for-byte"
    );
}
...
/// port follows: a crash-class JS failure becomes a named stderr line
/// here, never a stack trace.
#[test]
...
/// repository: a bare copy, with no `.git` of its own, is not enough.
...
// ---- check-backlog's warn/ok, exit-1, and schema-early-return arms.
...
// ---- `list`'s filters beyond `--open`.

/// `list --status`/`--section`/`--type`/`--milestone`/`--batch` over the
/// `mini` fixture, each matching the frozen JS exactly.
#[test]
...
// ---- the ruled argv deviation: the binary parses argv with clap, so a
// flag the frozen JS's `parseArgs` silently ignored, coerced to a bare
// `true`, or let a duplicate override is a named clap usage error at
// exit 2 instead. Each test below names the JS's own (success-path)
// answer.
...
/// exception among these six, so only the exit code is pinned, not
/// clap's wording.
#[test]
...
// ---- `batch` and `set`'s own argv-deviation pins.
...
// ---- the ruled clap argv deviation, for `check-backlog`: probing an
// unknown flag or unexpected positional live (`tools/backlog.mjs check
// --bogus`/`extra`) finds the same JS-ignores-it-and-succeeds divergence
// every other flat command has.
