//! Backlog command parity tests (batch 17 T2): five backlog slices
//! (`list --open`, `get HR-052`, `batch 14`, `check`, `set`), byte-compared
//! verbatim against the compiled binary, plus the CLI's usage-error and
//! load-failure arms (`tools/backlog.mjs`'s `main`, ported).
//!
//! The five slices moved from the frozen fixture corpus (`tests/corpus/
//! backlog/`) to reviewed, Rust-generated goldens (`tests/goldens/
//! backlog/`, regenerated by `cargo run --bin gen-goldens`) at batch 20 T3
//! (HR-047, docs/specs/2026-09-07-batch-20-phase5.md §2's boundary
//! correction; design.md §5.42): `tests/corpus/**` retires with the rest
