...
describe('the live check command over the frozen corpus', () => {
  ...
  /**
   * The exact `{ stdout, stderr, exit }` `template/tools/kb.mjs check`
   * produces for each portable fixture slice, inlined rather than read
   * from `tests/corpus/check/<slice>.json` (batch 18 T5, docs/specs/
   * 2026-09-05-batch-18-phase3.md §3): that directory moved to reviewed,
   * Rust-generated goldens under `tests/goldens/check/`, which now pin the
   * BINARY's rewritten `houserules render` hint text -- a different, and
   * for `mini-stale`, DIFFERENT-BYTES expectation than this still-frozen
   * JS engine's own, unchanged `tools/kb.sh render` hint. `template/tools/
   * kb.mjs` never rewrites (spec §4: shipped-but-inert through phase 4),
   * so its own expected output is this fixed, hand-verified literal, not
   * an external file that could drift out from under it.
   */
  const EXPECTED_CHECK = {
    mini: { stdout: 'knowledge: ok\n', stderr: '', exit: 0 },
    root: { stdout: 'knowledge: ok\n', stderr: '', exit: 0 },
    'mini-bad': {
      stdout: '',
      stderr:
        'knowledge/areas.json: area "cli" is missing\n' +
        'knowledge/areas.json: unknown area "bogus"\n' +
        'knowledge/entries.json.entries[9].summary: longer than 160 characters\n' +
        'knowledge/entries.json.entries[10].body: must be array\n' +
        'knowledge/entries.json.entries[11].source.date: must match ^\\d{4}-\\d{2}-\\d{2}$\n' +
        'knowledge/entries.json.entries[12].summary: shorter than 1\n' +
        'knowledge/entries.json.entries[13].check.body_line_max: below 1\n' +
        'knowledge/entries.json.entries[14].tags: items must be unique\n' +
        'knowledge/entries.json.entries[15]: missing "tags"\n' +
        'knowledge/entries.json.entries[16]: unknown field "nickname"\n' +
        'knowledge/entries.json.entries[17].kind: must be one of "rule", "invariant", "gotcha", "procedure", "decision", "history"\n' +
        'knowledge/entries.json entries.dup: duplicate id (also in knowledge/entries.json)\n' +
        'knowledge/entries.json wrongprefix.oops: id must start with "entries."\n' +
        'knowledge/entries.json entries.bad-standing: standing needs kind rule or invariant and area global or process\n' +
        'knowledge/entries.json entries.bad-see: see "entries.nonexistent" does not exist\n' +
        'knowledge/entries.json entries.bad-verify: verify path "nonexistent/path-xyz.txt" does not exist\n' +
        'knowledge/entries.json entries.bad-check-fields: check "grep-absent" needs "files"\n' +
        'knowledge/entries.json entries.bad-check-commits: check "commits" needs "subject", "body_absent", or "body_line_max"\n' +
        'knowledge/entries.json entries.bad-regex: check pattern is not a valid regex (Invalid regular expression: /(/: Unterminated group)\n' +
        'knowledge/mismatch.json: topic "not-mismatch" must equal the file name "mismatch"\n',
      exit: 1,
    },
    // batch 18 T5: the mini-stale fixture's checked-in `standing-rules.md`
    // was rewritten to match the CURRENT binary's `houserules render`
    // header, so `check_parity.rs`'s own golden sees it as up to date.
    // This still-frozen JS engine computes `tools/kb.sh render` instead,
    // so the SAME file now reads as stale to it too -- a fourth
    // `generated file is out of date` line neither engine had reason to
    // produce before the two diverged.
    'mini-stale': {
      stdout: '',
      stderr:
        '.claude/rules/standing-rules.md: generated file is out of date (run tools/kb.sh render)\n' +
        '.claude/rules/tools.md: generated file is out of date (run tools/kb.sh render)\n' +
        '.claude/skills/project-knowledge/SKILL.md: generated file is out of date (run tools/kb.sh render)\n' +
        '.claude/rules/stray.md: not generated by kb; remove it\n' +
        'CLAUDE.md: 14257 bytes, budget 12288\n' +
        '.claude/rules/tools.md: 173 lines, budget 160\n' +
        '.claude/skills/project-knowledge/SKILL.md: missing\n',
      exit: 1,
    },
  };
  ...
  // commit) and asserts byte parity with `EXPECTED_CHECK` above.
  //
  // `mini-bad` fails schema/shape validation before checkBase ever reaches
  // the render-staleness comparison, so it is unaffected by the batch 18
  // T5 divergence below and is checked as-is; `mini-stale`'s deliberately
  // wrong `tools.md` must never be regenerated (that mismatch IS the
  // fixture's point), so it is checked as-is too, with the one extra
  // stale line `EXPECTED_CHECK` already documents.
  it.each(['mini-bad', 'mini-stale'])('matches the frozen %s slice', (slice) => {
    ...
    git(root, 'init', '-q', '-b', 'main');
    commit(root, 'chore: init');
    expect(runKbCheck(root)).toEqual(EXPECTED_CHECK[slice]);
  });

  // `mini`, unlike `mini-bad`/`mini-stale`, carries no deliberately wrong
  // content: its checked-in `.claude/rules/*` now match the CURRENT
  // binary's `houserules render` header (`tests/corpus/fixtures/mini`'s
  // own files, shared with `check_parity.rs`'s golden), so this still-
  // frozen JS engine sees them as stale on sight. Rendering with the SAME
  // live `kb.mjs` first repairs that (mirroring `check_parity.rs`'s own
  // root test, which renders with the CURRENT binary before checking, for
  // the identical reason) before asserting the clean result this
  // describe block's whole point is to keep proving.
  it('matches the frozen mini slice after a live render repairs the header mismatch', () => {
    const root = scratchDir('kb-live-check-');
    ...
    git(root, 'init', '-q', '-b', 'main');
    commit(root, 'chore: init');
    const renderResult = spawnSync(process.execPath, [KB_MJS, 'render'], { cwd: root, encoding: 'utf8' });
    expect(renderResult.status).toBe(0);
    expect(runKbCheck(root)).toEqual(EXPECTED_CHECK.mini);
  });

  // Batch 18 T6 (docs/specs/2026-09-05-batch-18-phase3.md §4): this
  // repository's own `.claude/rules/*.md` and skill now carry the BINARY's
  // "houserules render" stamp (the dogfood flip) -- this still-frozen JS
  // engine's own render disagrees forever after, the identical fact
  // `check_parity.rs`'s own root test already handles on the Rust side by
  // rendering a worktree copy before checking it (that test's own doc has
  // the account). `HEAD`, not a frozen historical sha: this test's whole
  // point is the CURRENT live tree, not a pinned past one. A worktree
  // (not a partial `cpSync`) matters here specifically because this
  // repository's own knowledge base carries `verify` entries naming files
  // all over the tree (`mise.toml`, `crates/houserules/src/install.rs`,
  // and more) that `checkBase` reads by path; only a full tracked-file
  // copy satisfies every one of them the way `REPO_ROOT` itself did.
  it("matches the frozen root slice's expectation against this repository's own live tree", () => {
    withFrozenWorktree('HEAD', (worktree) => {
      const renderResult = spawnSync(process.execPath, [KB_MJS, 'render'], {
        cwd: worktree,
        encoding: 'utf8',
      });
      expect(renderResult.status).toBe(0);
      expect(runKbCheck(worktree)).toEqual(EXPECTED_CHECK.root);
    });
  });
  ...
  /**
   * Reads one frozen `tests/fixtures/kb-mjs-frozen-for/<relativePath>`
   * capture -- `for`'s two root slices, moved here from `tests/corpus/
   * knowledge/` at batch 18 T5 fix round 1 (spec §3 boundary
   * clarification, commit 878265b). `template/tools/kb.mjs` never
   * rewrites (it stays shipped-but-inert through phase 4), so its own
   * `for` output is this fixed, unchanging capture; `tests/corpus/` itself
   * dropped both files, since `tools/make-corpus.mjs` now regenerates
   * only the slices that stay genuinely frozen-JS-pinned, and
   * `corpus.test.mjs`'s regeneration test would otherwise flag these two
   * as extra, unmanaged files.
   */
  function readFrozenForCapture(relativePath) {
    return JSON.parse(
      readFileSync(
        new URL(`./fixtures/kb-mjs-frozen-for/${relativePath}`, import.meta.url),
        'utf8',
      ),
    );
  }
  ...
  // `for`'s own slices read from the frozen fixture directly (this describe
  // block's own `readFrozenForCapture` doc has the account of why they no
  // longer live under `tests/corpus/`).
  it.each([
    ['for-tools-kb-mjs', ['for', 'tools/kb.mjs']],
    ['for-tools-kb-mjs-full', ['for', 'tools/kb.mjs', '--full']],
  ])('%s matches the frozen knowledge corpus slice', (slice, args) => {
    const expected = readFrozenForCapture(`${slice}.json`);
    withFrozenWorktree(frozenSha(), (worktree) => {
      expect(runKb(args, worktree)).toEqual({
        stdout: expected.stdout,
        stderr: expected.stderr,
