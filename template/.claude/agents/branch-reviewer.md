...
Run `houserules audit --base <BASE> --head <HEAD> --workspace <WORKSPACE> --json <WORKSPACE>/branch-audit.json` and `houserules check-knowledge`. Declare a spec-booked interim fail once with `--sanctioned <rule>=<ref>` instead of narrating it by hand. Judge every `open` row over the whole diff; `rule_adherence` holds every row judged. File failures as issues with the standard severities (standing rule critical, area rule important, warn minor). With `--workspace` a report-field row is judged from the task reports; a skipped row means the workspace was not passed — re-run the audit with it. After the workspace audit, compare the report count the audit's evidence names against the workspace's `task-*-report.json` listing; a mismatch invalidates every report-field row. When an area's files carry no code change, judge its rows from the audit's `area_files` and cite that list as the evidence for each row. Compare the latest `templates` blob ids in `.claude/evals/record.json` with `git rev-parse HEAD:<path>` for `implementer.md` and `task-reviewer.md`; a mismatch is a critical finding under `process.evals-rerun`.
